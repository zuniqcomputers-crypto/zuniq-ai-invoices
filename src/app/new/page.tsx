...
export default function NewInvoice() {
  ...
  const [invoiceData, setInvoiceData] = useState<InvoiceData>({
    ...
    business_logo_url: "", signature_url: "", qr_code_data: "", client_name: "", client_email: "",
    client_phone: "", client_address: "", items: [], subtotal: 0, tax_percentage: -1, discount: -1,
    total: 0, currency: "", due_date: "", issue_date: new Date().toISOString().split("T")[0], notes: "",
  });

  const [messages, setMessages] = useState([
    { sender: "ai", text: "Hey! 👋 Describe your invoice. For example: 'Invoice for Ali, 3 design pages, due in 7 days, 20% advance'." },
  ]);
  ...
  const quickActions = [
    { label: "Create Invoice", icon: <Icon.create />, action: () => window.location.href = "/new" },
    { label: "Invoice History", icon: <Icon.history />, action: () => window.location.href = "/dashboard" },
    { label: "Upload Logo", icon: <Icon.upload />, action: () => document.getElementById("logoUpload")?.click() },
    { label: "QR Code", icon: <Icon.qr />, action: () => document.getElementById("qrUpload")?.click() },
    { label: useSmartAI ? "✨ Smart AI (On)" : "💡 Smart AI (Off)", icon: useSmartAI ? <Icon.smartOn /> : <Icon.smart />, action: () => setUseSmartAI(!useSmartAI) },
    { label: "Templates", icon: <Icon.template />, action: () => setShowTemplates(true) },
    { label: "Export PDF", icon: <Icon.export />, action: () => { if (isComplete) handleFinalize(); else alert("Complete the invoice first."); } },
    { label: "Clients", icon: <Icon.clients />, action: () => alert("Client database coming soon!") },
    { label: "Preview", icon: <Icon.preview />, action: () => setShowPreview(true) },
  ];
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-900">
        <div className="text-center bg-slate-800 p-8 rounded-3xl shadow-2xl max-w-md mx-4">
          ...
          <div className="space-y-3">
            <Link href="/dashboard" className="block w-full py-3 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-500 transition">Go to Dashboard</Link>
    ...
    <div className="flex flex-col h-screen bg-slate-900">
      ...
      <header className="bg-slate-800/80 backdrop-blur-md border-b border-slate-700 px-4 sm:px-6 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setShowPreview(true)} className="text-sm px-4 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-white font-medium transition">📄 Preview</button>
          {isComplete && <button onClick={handleFinalize} className="text-sm px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition">Finalize ✨</button>}
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 space-y-5">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"} animate-fade-in`}>
            <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${msg.sender === "user" ? "bg-indigo-600 text-white rounded-br-md" : "bg-slate-800 text-slate-200 rounded-bl-md border border-slate-700"}`}>{msg.text}</div>
          </div>
        ))}
        {thinking && (
          <div className="flex justify-start animate-fade-in">
            <div className="bg-slate-800 text-slate-400 text-sm px-4 py-3 rounded-2xl rounded-bl-md border border-slate-700 flex items-center gap-2">
              <span className="flex gap-1"><span className="h-2 w-2 bg-slate-500 rounded-full animate-bounce" style={{ animationDelay: "0ms" }}></span><span className="h-2 w-2 bg-slate-500 rounded-full animate-bounce" style={{ animationDelay: "150ms" }}></span><span className="h-2 w-2 bg-slate-500 rounded-full animate-bounce" style={{ animationDelay: "300ms" }}></span></span>
        ...
        {useSmartAI && (
          <div className="flex items-center justify-center mb-2">
            <span className="inline-flex items-center gap-1.5 bg-emerald-500/10 text-emerald-400 px-3 py-1 rounded-full text-xs font-medium">
              <Icon.smartOn /> Smart AI is active
            </span>
          </div>
        )}
        <div className="flex items-center gap-2">
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSend()} placeholder="Describe your invoice..." className="flex-1 h-12 px-4 rounded-xl border border-slate-600 bg-slate-700 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition" disabled={thinking} />
          <button onClick={handleSend} disabled={thinking} className="h-12 w-12 rounded-xl bg-indigo-600 text-white flex items-center justify-center hover:bg-indigo-500 active:scale-95 transition disabled:opacity-50">➤</button>
        </div>
      </div>

      {/* Floating + button */}
      <div className="fixed bottom-20 right-4 z-40">
        <button onClick={() => setShowQuickActions(!showQuickActions)} className="h-12 w-12 rounded-full bg-indigo-600 text-white shadow-lg flex items-center justify-center hover:scale-105 transition">
        ...
        {showQuickActions && (
          <div className="absolute bottom-16 right-0 bg-slate-800 rounded-xl shadow-2xl border border-slate-700 p-2 w-56 animate-fade-in">
            {quickActions.map((action, idx) => (
              <button key={idx} onClick={() => { action.action(); setShowQuickActions(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-200 hover:bg-slate-700 rounded-lg transition">
      ...
      {showPreview && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowPreview(false)}>
          <div className="bg-slate-800 rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-4 sm:p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold text-white">📄 Invoice Preview</h2><button onClick={() => setShowPreview(false)} className="text-slate-400 hover:text-white text-2xl">&times;</button></div>
      ...
      {showTemplates && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowTemplates(false)}>
          <div className="bg-slate-800 rounded-2xl shadow-2xl max-w-lg w-full p-6 sm:p-8" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold text-white">📋 Invoice Templates</h2><button onClick={() => setShowTemplates(false)} className="text-slate-400 hover:text-white text-2xl">&times;</button></div>
            <div className="grid gap-3">
              {templates.map((t, i) => (
                <button key={i} onClick={() => { setInvoiceData({ ...invoiceData, ...t.data } as InvoiceData); setShowTemplates(false); setMessages([...messages, { sender: "ai", text: `Loaded "${t.name}" template.` }]); }} className="flex items-center gap-3 p-4 rounded-xl border border-slate-700 hover:border-indigo-500 hover:bg-indigo-500/10 transition text-left">
                  <div className="h-10 w-10 rounded-lg bg-indigo-500/10 text-indigo-400 flex items-center justify-center font-bold">{t.name.charAt(0)}</div>
                  <div><p className="font-semibold text-white">{t.name}</p><p className="text-xs text-slate-400">{t.description}</p></div>
                </button>
              ))}
            </div>
          </div>
